        ...
        public void onReceive(Context context, Intent intent) {
            if (intent.getAction() != null) {
                if (intent.getAction().equals(Constants.ACTION.BROADCAST_MESSAGE)) {
                    ...
                    String theSpeaker = intent.getStringExtra(Constants.KEY.SPEAKER);
                    displaySpeaker(theSpeaker);
                }
            }
        }
    };

    private void displaySpeaker(final String value){
        getActivity().runOnUiThread(new Runnable() {
            @Override
            public void run() {
                speaker.setText(String.format(Locale.getDefault(), getString(R.string.speaker), value));
            }
        });
    }
