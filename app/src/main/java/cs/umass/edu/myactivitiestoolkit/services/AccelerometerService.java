...
public class AccelerometerService extends SensorService implements SensorEventListener {
        ...
        mClient.registerMessageReceiver(new MessageReceiver(Constants.MHLClientFilter.ACTIVITY_DETECTED) {
            ...
            protected void onMessageReceived(JSONObject json) {
                ...
                try {
                    ...
                    broadcastActivity(Integer.parseInt(activity));
                } catch (JSONException e) {
                    e.printStackTrace();
                    return;
                }
    ...
    protected void registerSensors() {
        ...
        mSensorManager.registerListener(this, mSensorManager.getDefaultSensor(Sensor.TYPE_STEP_DETECTOR), mSensorManager.SENSOR_DELAY_NORMAL);
        ...
        mSensorManager.registerListener(mStepDetector, mSensorManager.getDefaultSensor(Sensor.TYPE_ACCELEROMETER), mSensorManager.SENSOR_DELAY_NORMAL);
    ...
    public void onSensorChanged(SensorEvent event) {
        if (event.sensor.getType() == Sensor.TYPE_ACCELEROMETER) {
            ...
            AccelerometerReading ar = new AccelerometerReading(mUserID, "MOBILE", "", timestamp_in_milliseconds, event.values);
            mClient.sendSensorReading(ar);
    ...
    /** Group 7
     * Broadcasts the step count computed by your step detection algorithm from Serve
     * to other application components, e.g. the main UI.
     */
    ...
    /** Group 7
     * Broadcasts the activity computed by your classifier
     * to other application components, e.g. the main UI.
     */
    public void broadcastActivity(int label) {
        Intent intent = new Intent();
        intent.putExtra(Constants.KEY.ACTIVITY, label);
