...
public class DBScan<T extends Clusterable<T>> {
    ...
    public List<Cluster<T>> cluster(final Collection<T> points) {
        ...
        for (final T p : points) {
            ...
            if (NeighborPts.size() < getMinPts())
                ...
                Cluster<T> group = new Cluster<T>();
                clusters.add(group);
                expandCluster(group, p, states, NeighborPts, points);
            }
        }

        for(int i = 0; i < clusters.size(); i ++){
            Cluster<T> currentCluster = clusters.get(i);
        }
    ...
    private void expandCluster(Cluster<T> cluster, final T p, final Map<T, State> states,
        ...
        for(int i = 0; i < neighborPts.size(); i ++){
            final T np = neighborPts.get(i);
            if (states.get(np) == State.UNVISITED) {
                states.put(np, State.VISITED);
                List<T> npNeighborPts = regionQuery(np, points);
                if (npNeighborPts.size() >= getMinPts())
                    addAsSet(neighborPts, npNeighborPts);
            }
