...
public class LocationsFragment extends Fragment {
    ...
    private void drawClusters(final Collection<Cluster<GPSLocation>> clusters){
        ...
        System.out.println("GG: clusters size - "+ clusters.size());
        int currentColor = 0;
        for(Cluster<GPSLocation> group: clusters){
            int cluster_size = group.getPoints().size();
            GPSLocation[] cluster_array = new GPSLocation[cluster_size];
            int color_index = 0;
            for(GPSLocation pt: group.getPoints()){
                /*
                Marker mark = map.addMarker(new MarkerOptions()
                        .position(new LatLng(pt.getLatitude(), pt.getLongitude())) //sets the latitude & longitude
                        .title("At " + LocationDAO.getISOTimeString(pt.getTimestamp()))); //display the time it occurred when clicked
                locationMarkers.add(mark);*/
                cluster_array[color_index++] = pt;
            }
            drawHullFromPoints(cluster_array,colors[currentColor]);
    ...
    private void runDBScan(GPSLocation[] locations, float eps, int minPts){
        ...
        DBScan clustering = new DBScan(eps, minPts);
        List<GPSLocation> location_list = new ArrayList<GPSLocation>();
        for(GPSLocation loc: locations){
            location_list.add(loc);
        }
        drawClusters(clustering.cluster(location_list));
    }
        ...
        client.registerMessageReceiver(new MessageReceiver() {
            ...
            protected void onMessageReceived(JSONObject json) {
                ...
                try {
                    ...
                    for (int locationIndex = 0; locationIndex < indexes.length; locationIndex++) {
                        int clusterNumber = indexes[locationIndex];
                        //TODO: Using the index of each location, generate a list of k clusters, then call drawClusters().
                        if(clusters.get(clusterNumber) == null)
                            clusters.put(clusterNumber, new Cluster<GPSLocation>());
                        Cluster<GPSLocation> currentCluster = clusters.get(clusterNumber);
                        currentCluster.addPoint(locations[locationIndex]);
                        clusters.put(clusterNumber, currentCluster);
                        //You may choose to use the Map defined above or find a different way of doing it.
                    }
                    List<Cluster<GPSLocation>> clusterList = new ArrayList<Cluster<GPSLocation>>();
                    for(Map.Entry<Integer, Cluster<GPSLocation>> e: clusters.entrySet()){
                        clusterList.add((Cluster<GPSLocation>)e.getValue());
                        for(GPSLocation loc: e.getValue().getPoints()){
