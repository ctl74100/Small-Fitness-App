...
public class HeartRateCameraView extends SurfaceView implements Callback, Camera.PreviewCallback {
    ...
    public void onPreviewFrame(byte[] data, Camera camera) {
        ...
        double mean = 0;
        //Calculate the mean of the area near the center
        for (int y = 30; y < height - 30; y++) {
            for (int x = 30; x < width - 30; x++) {
                mean += Color.red(pixels[x * y + x]);
            }
        }
        mean /= ((height - 60) * (width - 60));


        for(PPGListener listen: listeners){
            listen.onSensorChanged(new PPGEvent(mean, System.currentTimeMillis ()));
        }
